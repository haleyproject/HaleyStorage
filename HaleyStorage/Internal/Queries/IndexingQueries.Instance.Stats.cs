                    ...
                    @"create temporary table tmp_stat_rebuild engine=InnoDB as
                      ...
                      from (
                          select 1 as node_type, ws.id as node_id, ws.id as workspace, cast(null as char(100)) as ext,
                                 (select count(*) from directory as dir where not exists(select 1 from dir_path hp join directory hd on hd.id = hp.ancestor where hp.descendant = dir.id and left(hd.name, 1) = '.') and dir.workspace = ws.id and dir.parent = 0 and dir.delete_state = 0) as active_folders,
                                 (select count(*) from directory as dir where not exists(select 1 from dir_path hp join directory hd on hd.id = hp.ancestor where hp.descendant = dir.id and left(hd.name, 1) = '.') and dir.workspace = ws.id and dir.parent = 0 and dir.delete_state > 0) as deleted_folders,
                          ...
                          select 2, dir.id, dir.workspace, cast(null as char(100)),
                                 (select count(*) from directory as child where not exists(select 1 from dir_path hp join directory hd on hd.id = hp.ancestor where hp.descendant = child.id and left(hd.name, 1) = '.') and child.workspace = dir.workspace and child.parent = dir.id and child.delete_state = 0),
                                 (select count(*) from directory as child where not exists(select 1 from dir_path hp join directory hd on hd.id = hp.ancestor where hp.descendant = child.id and left(hd.name, 1) = '.') and child.workspace = dir.workspace and child.parent = dir.id and child.delete_state > 0),
                                 (select count(distinct doc.id) from document as doc inner join doc_version as dv on dv.parent = doc.id and dv.sub_ver = 0 and dv.delete_state = 0 inner join version_info as vi on vi.id = dv.id where not exists(select 1 from dir_path hp join directory hd on hd.id = hp.ancestor where hp.descendant = dir.id and left(hd.name, 1) = '.') and doc.parent = dir.id and doc.delete_state = 0 and (coalesce(vi.flags, 0) & 64) <> 0),
                                 (select count(*) from document as doc where not exists(select 1 from dir_path hp join directory hd on hd.id = hp.ancestor where hp.descendant = dir.id and left(hd.name, 1) = '.') and doc.parent = dir.id and doc.delete_state > 0),
                                 (select count(*) from document as doc inner join doc_version as dv on dv.parent = doc.id and dv.sub_ver = 0 and dv.delete_state = 0 inner join version_info as vi on vi.id = dv.id where not exists(select 1 from dir_path hp join directory hd on hd.id = hp.ancestor where hp.descendant = dir.id and left(hd.name, 1) = '.') and doc.parent = dir.id and doc.delete_state = 0 and (coalesce(vi.flags, 0) & 64) <> 0),
                                 (select count(*) from document as doc inner join doc_version as dv on dv.parent = doc.id and dv.sub_ver = 0 and dv.delete_state > 0 inner join version_info as vi on vi.id = dv.id where not exists(select 1 from dir_path hp join directory hd on hd.id = hp.ancestor where hp.descendant = dir.id and left(hd.name, 1) = '.') and doc.parent = dir.id and (coalesce(vi.flags, 0) & 64) <> 0),
                                 (select count(*) from document as doc inner join doc_version as dv on dv.parent = doc.id and dv.sub_ver > 0 and dv.delete_state = 0 inner join version_info as vi on vi.id = dv.id where not exists(select 1 from dir_path hp join directory hd on hd.id = hp.ancestor where hp.descendant = dir.id and left(hd.name, 1) = '.') and doc.parent = dir.id and doc.delete_state = 0 and (coalesce(vi.flags, 0) & 64) <> 0),
                                 (select count(*) from document as doc inner join doc_version as dv on dv.parent = doc.id and dv.sub_ver > 0 and dv.delete_state > 0 inner join version_info as vi on vi.id = dv.id where not exists(select 1 from dir_path hp join directory hd on hd.id = hp.ancestor where hp.descendant = dir.id and left(hd.name, 1) = '.') and doc.parent = dir.id and (coalesce(vi.flags, 0) & 64) <> 0),
                    ...
                    @"create temporary table tmp_stat_rebuild engine=InnoDB as
                      ...
                      from (
                          select 2 as node_type, d.parent as node_id, d.workspace,
                                 ...
                                 case when not exists(select 1 from dir_path hp join directory hd on hd.id = hp.ancestor where hp.descendant = d.parent and left(hd.name, 1) = '.') then count(distinct case when dv.sub_ver = 0 and d.delete_state = 0 and dv.delete_state = 0 then d.id end) else 0 end as active_docs,
                                 case when not exists(select 1 from dir_path hp join directory hd on hd.id = hp.ancestor where hp.descendant = d.parent and left(hd.name, 1) = '.') then count(distinct case when dv.sub_ver = 0 and d.delete_state > 0 then d.id end) else 0 end as deleted_docs,
                                 case when not exists(select 1 from dir_path hp join directory hd on hd.id = hp.ancestor where hp.descendant = d.parent and left(hd.name, 1) = '.') then sum(case when dv.sub_ver = 0 and d.delete_state = 0 and dv.delete_state = 0 then 1 else 0 end) else 0 end as active_versions,
                                 case when not exists(select 1 from dir_path hp join directory hd on hd.id = hp.ancestor where hp.descendant = d.parent and left(hd.name, 1) = '.') then sum(case when dv.sub_ver = 0 and dv.delete_state > 0 then 1 else 0 end) else 0 end as deleted_versions,
                                 case when not exists(select 1 from dir_path hp join directory hd on hd.id = hp.ancestor where hp.descendant = d.parent and left(hd.name, 1) = '.') then sum(case when dv.sub_ver > 0 and d.delete_state = 0 and dv.delete_state = 0 then 1 else 0 end) else 0 end as active_thumbs,
                                 case when not exists(select 1 from dir_path hp join directory hd on hd.id = hp.ancestor where hp.descendant = d.parent and left(hd.name, 1) = '.') then sum(case when dv.sub_ver > 0 and dv.delete_state > 0 then 1 else 0 end) else 0 end as deleted_thumbs,
