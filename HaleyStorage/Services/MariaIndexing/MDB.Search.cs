        ...
        public async Task<IFeedback<VaultFolderBrowseResponse>> SearchItems(IVaultReadRequest request, string searchTerm, VaultSearchMode searchMode, string extension = null, bool recursive = false, int page = 1, int pageSize = 50, bool includeAll = false, VaultFolderSortMode sort = VaultFolderSortMode.Id, VaultSortDirection direction = VaultSortDirection.Asc, VaultFolderItemKind kind = VaultFolderItemKind.Both, bool includeTotals = true) {
            ...
            try {
                ...
                if (directoryId < 1) {
                    ...
                    totalDirs  = !includeTotals || kind == VaultFolderItemKind.Files ? 0 : await _agw.ScalarAsync<long?>(moduleCuid, includeAll ? INSTANCE.SEARCH.COUNT_DIRS_ALL_INCLUDE_DELETED : INSTANCE.SEARCH.COUNT_DIRS_ALL,  default, ("@allow_hidden", request.AllowHiddenDirectories), (WSPACE, wsId), (VALUE, likePattern), (EXT, extParam)) ?? 0;
                    totalFiles = !includeTotals || kind == VaultFolderItemKind.Folders ? 0 : await _agw.ScalarAsync<long?>(moduleCuid, includeAll ? INSTANCE.SEARCH.COUNT_FILES_ALL_INCLUDE_DELETED : INSTANCE.SEARCH.COUNT_FILES_ALL, default, ("@allow_hidden", request.AllowHiddenDirectories), (WSPACE, wsId), (VALUE, likePattern), (EXT, extParam)) ?? 0;
                    rows       = await _agw.RowsAsync(moduleCuid, ApplyFolderListingOptions(includeAll ? INSTANCE.SEARCH.ITEMS_ALL_INCLUDE_DELETED : INSTANCE.SEARCH.ITEMS_ALL, "sr", sort, direction, kind), default, ("@allow_hidden", request.AllowHiddenDirectories), (WSPACE, wsId), (VALUE, likePattern), (EXT, extParam), (LIMIT_ROWS, fetchSize), (OFFSET_ROWS, offset));
                } else if (!recursive) {
                    // Scope: direct children of a specific directory.
                    totalDirs  = !includeTotals || kind == VaultFolderItemKind.Files ? 0 : await _agw.ScalarAsync<long?>(moduleCuid, includeAll ? INSTANCE.SEARCH.COUNT_DIRS_IN_DIR_INCLUDE_DELETED : INSTANCE.SEARCH.COUNT_DIRS_IN_DIR,  default, ("@allow_hidden", request.AllowHiddenDirectories), (WSPACE, wsId), (PARENT, directoryId), (VALUE, likePattern), (EXT, extParam)) ?? 0;
                    totalFiles = !includeTotals || kind == VaultFolderItemKind.Folders ? 0 : await _agw.ScalarAsync<long?>(moduleCuid, includeAll ? INSTANCE.SEARCH.COUNT_FILES_IN_DIR_INCLUDE_DELETED : INSTANCE.SEARCH.COUNT_FILES_IN_DIR, default, ("@allow_hidden", request.AllowHiddenDirectories), (WSPACE, wsId), (PARENT, directoryId), (VALUE, likePattern), (EXT, extParam)) ?? 0;
                    rows       = await _agw.RowsAsync(moduleCuid, ApplyFolderListingOptions(includeAll ? INSTANCE.SEARCH.ITEMS_IN_DIR_INCLUDE_DELETED : INSTANCE.SEARCH.ITEMS_IN_DIR, "sr", sort, direction, kind), default, ("@allow_hidden", request.AllowHiddenDirectories), (WSPACE, wsId), (PARENT, directoryId), (VALUE, likePattern), (EXT, extParam), (LIMIT_ROWS, fetchSize), (OFFSET_ROWS, offset));
                } else {
                    // Scope: recursive subtree of a directory (WITH RECURSIVE CTE).
                    totalDirs  = !includeTotals || kind == VaultFolderItemKind.Files ? 0 : await _agw.ScalarAsync<long?>(moduleCuid, includeAll ? INSTANCE.SEARCH.COUNT_DIRS_RECURSIVE_INCLUDE_DELETED : INSTANCE.SEARCH.COUNT_DIRS_RECURSIVE,  default, ("@allow_hidden", request.AllowHiddenDirectories), (WSPACE, wsId), (PARENT, directoryId), (VALUE, likePattern), (EXT, extParam)) ?? 0;
                    totalFiles = !includeTotals || kind == VaultFolderItemKind.Folders ? 0 : await _agw.ScalarAsync<long?>(moduleCuid, includeAll ? INSTANCE.SEARCH.COUNT_FILES_RECURSIVE_INCLUDE_DELETED : INSTANCE.SEARCH.COUNT_FILES_RECURSIVE, default, ("@allow_hidden", request.AllowHiddenDirectories), (WSPACE, wsId), (PARENT, directoryId), (VALUE, likePattern), (EXT, extParam)) ?? 0;
                    rows       = await _agw.RowsAsync(moduleCuid, ApplyFolderListingOptions(includeAll ? INSTANCE.SEARCH.ITEMS_RECURSIVE_INCLUDE_DELETED : INSTANCE.SEARCH.ITEMS_RECURSIVE, "sr", sort, direction, kind), default, ("@allow_hidden", request.AllowHiddenDirectories), (WSPACE, wsId), (PARENT, directoryId), (VALUE, likePattern), (EXT, extParam), (LIMIT_ROWS, fetchSize), (OFFSET_ROWS, offset));
                }
